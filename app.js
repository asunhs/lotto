...
require('app').directive('stackChart', /* @ngInject */ ["chartOption", function (chartOption) {
    return {
        restrict: 'E',
        replace: true,
        scope: {
...
var Analyzer = (function () {
    ...
    Analyzer.prototype.average = function (lottos, n) {
        ...
        return this.periods.map(function (period) { return new MovingAvg(period, spreadLotto[n]).toArray(); });
    };
    ...
    Analyzer.prototype.buildDataByBalls = function (record) {
        return Dataset.build(balls.map(function (ball) { return ball.toString(); }), [record]);
    };
    Analyzer.prototype.analyzeLast = function () {
        return this.lasts.map(this.buildDataByBalls);
    };
    Analyzer.prototype.analyzeDifference = function (origin, target) {
        var lhs = this.lasts[origin], rhs = this.lasts[target];
        return this.buildDataByBalls(lhs.map(function (l, index) { return Math.floor((l - rhs[index]) * 1000) / 1000; }));
    };
    return Analyzer;
})();
...
}]).controller('LottoCtrl', /* @ngInject */ ["$scope", "chartLast", "periods", function ($scope, chartLast, periods) {
...
},{"./rule/rule.js":8}]},{},["app",2,3,4,8,10,5,6,7,9,1]);
